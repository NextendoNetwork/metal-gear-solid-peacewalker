...
func startTURN(recorder *probeRecorder, listenAddress, relayAddress, username, password, realm string) (*turnServer, error) {
	...
	var generator turn.RelayAddressGenerator = &turn.RelayAddressGeneratorStatic{
		RelayAddress: relayIP,
		Address:      relayIP.String(),
	}
	if minPort, maxPort := envInt("NPLN_TURN_RELAY_MIN_PORT", 0), envInt("NPLN_TURN_RELAY_MAX_PORT", 0); minPort > 0 && maxPort >= minPort {
