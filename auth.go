...
func gatedIdentity(ext *authpb.ExternalIdToken, tenant string) (uint64, string, error) {
	...
	// 2b. Unsigned bare-PID test identity from an unlinked client
	if pid, ok := testPIDFromIDToken(ext); ok {
		log.Printf("[NPLN Auth] test pid=%d (unsigned, bare-PID range)", pid)
		acc, err := accountFriends(pid)
		if err == nil {
			return pid, tenant + "/users/" + acc.UserID, nil
		}
