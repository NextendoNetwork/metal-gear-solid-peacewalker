...
const (
	testPIDMin = 1800000000
	testPIDMax = 1809999999
)

// testPIDFromIDToken reads the unsigned "npid" claim an unlinked client sends. Only PIDs in the
// bare-PID test range are accepted, and NPLN_ALLOW_TEST_PID=0 turns the path off.
func testPIDFromIDToken(ext *authpb.ExternalIdToken) (uint64, bool) {
	if ext == nil || envOr("NPLN_ALLOW_TEST_PID", "1") == "0" {
		return 0, false
	}
	segments := strings.Split(ext.GetNsaIdToken(), ".")
	if len(segments) < 2 {
		return 0, false
	}
	payload, err := base64.RawURLEncoding.DecodeString(strings.TrimRight(segments[1], "="))
	...
	if json.Unmarshal(payload, &claims) != nil || claims.Nnex != "" || claims.Npid == "" {
		return 0, false
	}
	pid, err := strconv.ParseUint(claims.Npid, 10, 64)
	if err != nil || pid < testPIDMin || pid > testPIDMax {
		return 0, false
	}
	return pid, true
}
